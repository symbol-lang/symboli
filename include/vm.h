...
typedef enum {
	...
	OP_MAKE_TYPED_ARR,	  /* a = type index; b = element count; pops N values,
							 builds typed array */
	OP_INDEX,			  /* pop index, pop array, push array[index]          */
	OP_INDEX_SET,		  /* pop value, index, array; array[i]=v; push value */
	OP_SET_ARR_ELEM_TYPE, /* a = type_pool index; peek TOS array, set its elem
							 type */
...
typedef struct Chunk {
	...
	int nsub, subcap;
	InterpEntry* interps;
	int ninterp, interpcap;
	ObjTemplate* objtmpls;
	int nobjt, objtcap;
	Type** type_pool;
	int ntype, typecap; /* runtime type constants */
